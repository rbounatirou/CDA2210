...
collection.load().then(()=>{
    ...
    document.querySelector('#downloadBt').addEventListener('click',()=>{exportData();})
...
function exportData(){
    ...
    let fileToSave = new Blob([JSON.stringify(loadDataWithFilters())], { type : "application/json"});
    // CREER UN LIEN SUR LEQUEL ON VA SIMULER LE CLICK AFIN DE LANCER LE TELECHARGEMENT
