...
INNER JOIN countries ON countries.country_code LIKE cities.country_code
INNER JOIN (SELECT cities.country_code, COUNT(*) AS nbCities 
FROM cities 
INNER JOIN countries ON cities.country_code = countries.country_code 
GROUP BY cities.country_code) AS c ON cities.country_code = c.country_code
WHERE cities.country_code LIKE countries.country_code
